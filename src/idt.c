...
void init_idt()
{
	idt_p.limit = (sizeof(struct idt_e) * 256) - 1;
	idt_p.base = (uint32_t)&idte;
	memset(&idte, 0, sizeof(struct idt_e) * 256);
	portb(0x20, 0x11);
	portb(0xA0, 0x11);
	portb(0x21, 0x20);
	portb(0xA1, 0x20);
...
void set_idt_gate(uint8_t num, uint32_t base, uint16_t sel, uint8_t flags)
{
	...
	idte[num].base_hight = (base >> 16) & 0xFFFF;
	idte[num].sel = sel;
	idte[num].always0 = 0;
